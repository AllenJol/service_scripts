...
function  install_required_packages(){
  ...
  yum -y install wget gcc-c++ numactl autoconf automake libaio-devel zlib zlib-devel ncurses ncurses-devel \
  tcp_wrappers-devel bison-devel bison build-essential libncurses5-dev cmake openssl openssl-devel >>/dev/null
}
