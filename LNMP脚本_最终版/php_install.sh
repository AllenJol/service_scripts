...
LCV_SOURCE="libiconv-1.14"
LCV_DOWN_URL="http://ftp.gnu.org/pub/gnu/libiconv/${LCV_SOURCE}.tar.gz"
...
function install_required_packages(){
  ...
  if [ $? -eq 0 ];then
    ...
    yum install -y gcc gcc-c++ make cmake automake autoconf gd gd-devel \
    zlib zlib-devel pcre pcre-devel libjpeg libjpeg-devel \
    libpng libpng-devel freetype freetype-devel libxml2 \
    libxml2-devel glibc glibc-devel glib2 glib2-devel bzip2 \
    bzip2-devel ncurses ncurses-devel curl curl-devel libcurl \
    libcurl-devel openssl openssl-devel openldap openldap-devel \
    openldap-clients kernel-devel libtool-libs libxslt libxslt-devel \
    gettext-devel libcap-devel php-mcrypt libmcrypt libmcrypt-devel \
    mhash mhash-devl recode-devel gmp-devel >>/dev/null
  else
...
function install_libiconv(){
  if [ ! -f "${DIR}/${LCV_SOURCE}.tar.gz" ];then
    cd ${DIR} && wget ${LCV_DOWN_URL}
    tar xf ${LCV_SOURCE}.tar.gz
    cd ${LCV_SOURCE} && ./configure --prefix=/usr/local/libiconv
...
if [ $? -eq 0 ];then
  echo "make php56 successfully." && sleep 2
else
  echo "make php56 Error,Please check it."
  exit 1
fi
make install >>/dev/null
if [ $? -eq 0 ];then
  echo "make install php56 successfully." && sleep 2
else
  echo "make install php56 Error,Please check it."
  exit 1
fi
#拷贝php配置文件、启动脚本等
...
make -j ${CPU_CORE} >>/dev/null
if [ $? -eq 0 ];then
  echo "make php71 successfully." && sleep 2
else
  echo "make php71 Error,Please check it."
  exit 1
fi
make install >>/dev/null
if [ $? -eq 0 ];then
  ...
  echo "make install php71 Error,Please check it."
  exit 1
fi
#拷贝php配置文件、启动脚本等
...
;;
*) 
  echo -e "\e[1;33mchoose php version error,please check\e[0m" && exit 1
;;
esac
}
